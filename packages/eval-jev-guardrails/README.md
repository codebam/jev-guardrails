...
Store an API key first (mode-600 config at `~/.config/eval-jev/config.json`):

```bash
# Any of the commands below can be run with npx:
npx @codebam/eval-jev-guardrails login --token eval_...
```
...
`$XDG_CONFIG_HOME/eval-jev/config.json`. `eval-jev login --token eval_...`
writes the file with mode `600`. Without `--token`, login tries the service's
GitHub device-flow endpoints when available, then falls back to a prompt.
...
eval-jev install dsh      [--profile <name>] [--dsh-home <path>]
...
reached. `credits` prints the remaining balance.
...
`/v1/evaluate`, `/v1/systemone`, `/v1/credits`, and `/v1/me`, and the Hermes
test drives the generated Python plugin against that fake via `python3`.
